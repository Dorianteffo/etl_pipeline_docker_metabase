import pandas as pd 
import logging 
from to_landing import create_conn, close_conn
...
def read_table(engine, table_name) :
    ...
    try: 
        df = pd.read_sql_query(f'SELECT * FROM landing_area."{table_name}"', engine)
        logger.info('Table read from the landing_area!!!!')
        return df 
    except Exception as e : 
        logger.error('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        logger.error(f'Enable to read data from landing_area!!!')
...
def data_modeling(df): 
    ...
    item_df = item_df.rename(columns = {'ITEM CODE' : 'ITEM_CODE', 'ITEM TYPE' : 'ITEM_TYPE', 'ITEM DESCRIPTION': 'ITEM_DESCRIPTION'})
    ...
    fact_table = df.merge(supplier_df, on = 'SUPPLIER') \
                    .merge(item_df, left_on="ITEM CODE", right_on="ITEM_CODE") \
                    .merge(date_df, on= ["YEAR", "MONTH"])[['ITEM_CODE', 'SUPPLIER_ID', 'DATE_ID',
...
def load_tables_staging(dict, engine):
    ...
    try :
        for df_name, value_dict in dict.items():
            ...
            logger.info(f'Importing {len(value_df)} rows from landing_area to staging_area.{df_name}')
            value_df.to_sql(df_name, engine, if_exists='replace', index=False, schema = 'staging_area')

            logger.info('!!!!!!!!')
            logger.info(f'Succesful load to {df_name}')
